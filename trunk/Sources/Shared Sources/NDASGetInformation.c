...
bool NDASGetInformationBySlotNumber(PNDASPhysicalDeviceInformation parameter) 
{
	...
	while(deviceUpNext && found == false) {
		...
		if ( 0 != strcmp(name, "com_ximeta_driver_NDASDevice")) {
			...
			IOObjectRelease(device); device = 0;
				...
				if(kNDASUnitStatusNotPresent != parameter->Units[unitNumber].status) {
					...
					if(CFDictionaryGetValueIfPresent(UnitProperties, CFSTR(NDASDEVICE_PROPERTY_KEY_UNIT_NUMBER_OF_SECTORS), (const void**)&cfnUnitCapacity)
					   && cfnUnitCapacity) {
						...
						CFNumberGetValue(cfnUnitCapacity, kCFNumberSInt64Type, &parameter->Units[unitNumber].capacity);
						parameter->Units[unitNumber].capacity *= DISK_BLOCK_SIZE;
					} else {
						parameter->Units[unitNumber].capacity = 0;
					}
...
togo:								
				IOObjectRelease(UnitDevice); UnitDevice = 0;
...
int NDASGetInformationGetCount(char *deviceClassName, int array[]) 
{	
	kern_return_t			kernResult;
	io_connect_t			dataPort;
	io_service_t			serviceObject;
	...
	// create Array.
//	array = malloc(sizeof(int) * numberOfDevices);
