...
void	com_ximeta_driver_NDASUnitDevice::setUnitDiskLocation(PUNIT_DISK_LOCATION location)
{
	...
	setProperty(NDASDEVICE_PROPERTY_KEY_UNIT_UNIT_DISK_LOCATION, *((UInt64 *)&fUnitDiskLocation), 64);
}
...
void	com_ximeta_driver_NDASUnitDevice::setConfiguration(UInt32 configuration)
{
...
UInt32	com_ximeta_driver_NDASUnitDevice::Configuration()
{
	return fConfiguration;
}

void	com_ximeta_driver_NDASUnitDevice::setStatus(UInt32 status)
{
...
UInt32	com_ximeta_driver_NDASUnitDevice::Status()
{
...
void	com_ximeta_driver_NDASUnitDevice::setCapacity(UInt64 sectors, UInt32 align)
{
...
UInt64	com_ximeta_driver_NDASUnitDevice::Capacity() 
{
	return fCapacity;
}

void	com_ximeta_driver_NDASUnitDevice::setNRRWHosts(UInt32 hosts)
{
	if (fNRRWHosts != hosts) {
		fNRRWHosts = hosts;
...
UInt32	com_ximeta_driver_NDASUnitDevice::NRRWHosts()
{
	return fNRRWHosts;
}

void	com_ximeta_driver_NDASUnitDevice::setNRROHosts(UInt32 hosts)
{
	if (fNRROHosts != hosts) {
...
UInt32	com_ximeta_driver_NDASUnitDevice::NRROHosts()
{
	return fNRROHosts;
}

void	com_ximeta_driver_NDASUnitDevice::setNumberOfSectors(UInt64 NumberOfSectors)
{
...
UInt64	com_ximeta_driver_NDASUnitDevice::NumberOfSectors()
{
	return fNumberOfSectors;
}

void	com_ximeta_driver_NDASUnitDevice::setDiskArrayType(UInt32 raidType)
{
	fDiskArrayType = raidType;
	setProperty(NDASDEVICE_PROPERTY_KEY_UNIT_DISK_ARRAY_TYPE, fDiskArrayType, 32);
}

UInt32	com_ximeta_driver_NDASUnitDevice::DiskArrayType() 
{
...
void print_dib_v2(PNDAS_DIB_V2 dib2) {
	...
	// Signature.
	DbgIOLogNC(DEBUG_MASK_NDAS_ERROR, ("print_dib_v2: Signature 0x%x\n", NDASSwap64LittleToHost(dib2->Signature)));
...
bool	com_ximeta_driver_NDASUnitDevice::checkDiskArrayType()
{
	...
	if (NumberOfSectors() <= 0) {
		DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("checkDiskArrayType: Sector size if 0!!!\n"));
	...
	DbgIOLog(DEBUG_MASK_RAID_INFO, ("checkDiskArrayType: Location of DIB_v2 %lld\n", location));
	
	// Set Capacity by default value.
	setCapacity((NumberOfSectors() - NDAS_BLOCK_SIZE_XAREA), NDAS_USER_SPACE_ALIGN);

    if (false == readSectorsOnce(location, 1, (char *)&dib_v2))
    {
        DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("checkDiskArrayType: Reading DIB V2 of X area is failed.\n"));
...
//	print_dib_v2(& dib_v2);
        ...
        && IS_DIB_CRC_VALID(crc32_calc, dib_v2))
    {
			   ...
			   NDAS_DIB_VERSION_MINOR_V2 < NDASSwap32LittleToHost(dib_v2.MinorVersion))) )
        {
			...
			DbgIOLog(DEBUG_MASK_RAID_TRACE, ("checkDiskArrayType: Unknown DIB_v2 Version Maj %d Mir %d\n",
   ...
   location = NumberOfSectors() + NDAS_BLOCK_LOCATION_DIB_V1;
    ...
    if (false == readSectorsOnce(location, 1, (char *)&dib_v1))
    {
	...
	if (NDAS_DIB_SIGNATURE_V1 == dib_v1.Signature) {
		...
		switch(dib_v1.DiskType) {
			...
			case NDAS_DIB_DISK_TYPE_MIRROR_SLAVE:
			{
				...
				memcpy(& pUnitDiskLocation0->MACAddr, dib_v1.EtherAddress, sizeof(pUnitDiskLocation0->MACAddr));
				pUnitDiskLocation0->UnitNumber = dib_v1.UnitNumber;
				pUnitDiskLocation0->VID = NDAS_VID_DEFAULT;
				
				memcpy(pUnitDiskLocation1->MACAddr, dib_v1.PeerAddress, sizeof(pUnitDiskLocation1->MACAddr));
...
void print_RMD(PNDAS_RAID_META_DATA rmd) {
	...
	// Signature.
	DbgIOLogNC(DEBUG_MASK_NDAS_ERROR, ("print_RMD: Signature 0x%x\n", NDASSwap64LittleToHost(rmd->Signature)));
...
bool	com_ximeta_driver_NDASUnitDevice::bind(UInt32								type, 
											   UInt32								sequence,
											   ...
											   bool									Sync)
{
	...
	switch(type) {
		...
		case NMT_RAID1R3:
		{
			...
			{
				if(userSize  <= ( ((UInt64)1 << count)) * NDAS_BLOCK_SIZE_BITMAP * NDAS_BIT_PER_OOS_BITMAP_BLOCK ) // 512 GB : 128 SPB
				{
	...
	for (int count = 0; count < NumberOfUnitDevices; count++) {
		memcpy(&(pDIB_v2->UnitDisks[count]), LogicalDevices[count]->UnitDevice()->UnitDiskLocation(), sizeof(UInt64));
	}
	...
	if (false == Sync) {
		...
		for (int count = 1; count < 10; count++) {
			if(!writeSectorsOnce(count, 1, (char *)buffer)) {
				DbgIOLog(DEBUG_MASK_RAID_ERROR, ("bind: Can't Write Apple Partition Info Location %lld\n", count));
				return false;
			}		
		}
	}
...
bool	com_ximeta_driver_NDASUnitDevice::ReadBitMapFromUnitDevice(PNDAS_OOS_BITMAP_BLOCK pBitmap, UInt32	BitMapSectorPoint)
{
	if (!pBitmap) {		
	...
	// Read BitMap.				
	UInt64	dirtyBitSector = NumberOfSectors() + NDAS_BLOCK_LOCATION_BITMAP;
...
bool	com_ximeta_driver_NDASUnitDevice::WriteBitMapToUnitDevice(PNDAS_OOS_BITMAP_BLOCK pBitmap, UInt32	BitMapSectorPoint) {
...
void	com_ximeta_driver_NDASUnitDevice::setRAIDFLAG(UInt32 flag, bool value) {
...
bool	com_ximeta_driver_NDASUnitDevice::RAIDFlag(UInt32 flag) {
	if (flag & fRAIDFlags) {
		return true;
	} else {
		return false;
...
void	com_ximeta_driver_NDASUnitDevice::setSequence(UInt32 sequence)
{
	fSequence = sequence;
}

UInt32	com_ximeta_driver_NDASUnitDevice::Sequence()
{
	return fSequence;
}

void	com_ximeta_driver_NDASUnitDevice::setSubUnitIndex(UInt32 index)
{
	fSubUnitIndex = index;
}

UInt32	com_ximeta_driver_NDASUnitDevice::SubUnitIndex()
{
