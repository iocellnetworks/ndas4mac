...
bool com_ximeta_driver_NDASRAIDUnitDeviceController::enqueueCommand(com_ximeta_driver_NDASCommand *command)
{
	...
	if (kNDASCommandTerminateThread == command->command()) {
		DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("enqueueCommand: Set Thread Terminated.\n"));
...
//	bool	bTerminate = false;
	UInt32	secondFailedTime = 0;	// Secs.
    ...
    do {
		...
		if(kNDASRAIDStatusRecovering == fProvider->RAIDStatus()
		   ...
		   && bIdle) {
			...
			if (kNDASRAIDStatusRecovering == fProvider->RAIDStatus()) {
				wait_time.tv_sec = 0;
				wait_time.tv_nsec = NDAS_MAX_WAIT_TIME_FOR_RAID_THREAD_RECOVERING;
			} else {
				wait_time.tv_sec = NDAS_MAX_WAIT_TIME_FOR_RAID_THREAD_NORMAL;
				wait_time.tv_nsec = 0;
			}
			...
			switch(sema_result) {
				case KERN_SUCCESS:
				{
					...
					switch(command->command()) {
						...
						{
							processSrbCompletedCommand(command);							
						}
							break;
						default:
...
void com_ximeta_driver_NDASRAIDUnitDeviceController::processSettingCommand(com_ximeta_driver_NDASCommand *command)
{

	PNPCommand	*pnpCommand = command->pnpCommand();
...
void com_ximeta_driver_NDASRAIDUnitDeviceController::processSrbCommand(com_ximeta_driver_NDASCommand *command)
{
	...
	switch ( pScsiCommand->cdb[0] ) {
		case kSCSICmd_INQUIRY:
		{
			...
			switch(fProvider->Type()) {
				case NMT_AGGREGATE:
					sprintf(buffer, "AGGREGATE_%d", fProvider->Index());
					break;
				case NMT_RAID0R2:
					sprintf(buffer, "RAID0R2_%d", fProvider->Index());
					break;
				case NMT_RAID1R3:
					sprintf(buffer, "RAID1R3_%d", fProvider->Index());
					break;
				default:
					sprintf(buffer, "UNSUPPRAID_%d", fProvider->Index());
					break;
			}
...
void com_ximeta_driver_NDASRAIDUnitDeviceController::processSrbCompletedCommand(com_ximeta_driver_NDASCommand *command)
{
	com_ximeta_driver_NDASCommand	*receivedCommand = fProvider->NDASCommand();
...
com_ximeta_driver_NDASCommand	*BuildSCSICommand(
												  UInt8					scsiCommandOperationCodes,
												  UInt64				location,
												  UInt32				sectors,
												  UInt32				blocksize,
...
void	com_ximeta_driver_NDASRAIDUnitDeviceController::processReadWriteCommand(com_ximeta_driver_NDASCommand *command)
{
	...
	UInt64	endSector = 0;
	UInt64	offset = pScsiCommand->BufferOffset;
	UInt32	reqSectors = 0;
	UInt64	reqLocation = 0;
	...
	switch(fProvider->Type()) {
			...
			for (int count = 0; count < fProvider->CountOfUnitDevices(); count++) {
				if (fProvider->UnitDeviceAtIndex(count)) {
					...
					if (reqLocation <= endSector) {
						...
						com_ximeta_driver_NDASCommand	*newCommand;
			...
			*/
			...
			for (int count = 0; count < fProvider->CountOfUnitDevices(); count++) {
				if (fProvider->UnitDeviceAtIndex(count)) {
					...
					UInt32	diskSequence;
					UInt64	newLocation;
					UInt64	locationPointer;
					UInt32	actualSectors = 0;
					UInt32	MDIndex;
					UInt32	subOffset;
		...
		case NMT_RAID1R3:
		{
			com_ximeta_driver_NDASCommand	*newCommand = NULL;
...
void com_ximeta_driver_NDASRAIDUnitDeviceController::InitAndSetBitmap()
{
	...
	if ( !fProvider->IsDirty()
		...
		// Update RMD.
		UInt64 location = GoodUnitDevice->NumberOfSectors() + NDAS_BLOCK_LOCATION_RMD;
