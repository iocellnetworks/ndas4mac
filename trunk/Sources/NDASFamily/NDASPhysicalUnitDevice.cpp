...
void	com_ximeta_driver_NDASPhysicalUnitDevice::setDevice(com_ximeta_driver_NDASDevice * device)
{
	if (fDevice) {
		fDevice->release();
	}
...
UInt32 com_ximeta_driver_NDASPhysicalUnitDevice::Type()
{
...
{
	setProperty(NDASDEVICE_PROPERTY_KEY_WRITE_RIGHT, fDevice->writeRight());

	return fDevice->writeRight();
}

void	com_ximeta_driver_NDASPhysicalUnitDevice::setSlotNumber(UInt32 slotNumber)
{
	fSlotNumber = slotNumber;
}

UInt32	com_ximeta_driver_NDASPhysicalUnitDevice::slotNumber()
{
	return fSlotNumber;
}

void	com_ximeta_driver_NDASPhysicalUnitDevice::setUnitNumber(UInt32 unitNumber)
{
...
UInt32	com_ximeta_driver_NDASPhysicalUnitDevice::unitNumber()
{
	return fUnitNumber;
}
...
bool com_ximeta_driver_NDASPhysicalUnitDevice::readSectorsOnce(UInt64 location, UInt32 sectors, char *buffer)
{
...
void com_ximeta_driver_NDASPhysicalUnitDevice::completeCommand(com_ximeta_driver_NDASCommand *command)
{
	if (UpperUnitDevice()) {
		UpperUnitDevice()->completeCommand(command);
	} else {
		DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("completeCommand: No Device set!!!\n"));
	}
}
...
com_ximeta_driver_NDASCommand* com_ximeta_driver_NDASPhysicalUnitDevice::PendingCommand()
{
...
UInt32	com_ximeta_driver_NDASPhysicalUnitDevice::updateStatus()
{
	UInt32	newStatus;
	UInt32	prevStatus;
	bool	bDataConnected;
	...
	if (!IsPresent()
		&& !bDataConnected) {
/*		DbgIOLog(DEBUG_MASK_NDAS_WARNING, ("updateStatus: Device is present. But not connected\n"));
		if (kNDASUnitStatusConnectedRW == Status())
		{
			DbgIOLog(DEBUG_MASK_NDAS_WARNING, ("updateStatus: But it was connected before. Reconnect!\n"));
			NDAS_clock_get_system_value(&fDisconnectedTime);
			newStatus = kNDASUnitStatusReconnectRW;
		}
		else
*/		{
			DbgIOLog(DEBUG_MASK_NDAS_WARNING, ("updateStatus: Was not connected. Give up!\n"));
		...
		switch(Status()) {
			...
			{
				if (!IsConnected()) {
					DbgIOLog(DEBUG_MASK_NDAS_WARNING, ("updateStatus: kNDASUnitStatusConnectedRW : !IsConnected()\n"));
					NDAS_clock_get_system_value(&fDisconnectedTime);
					newStatus = kNDASUnitStatusReconnectRW;
				} else if (kNDASUnitConfigurationMountRW != Configuration()) {
					DbgIOLog(DEBUG_MASK_NDAS_WARNING, ("updateStatus: kNDASUnitStatusConnectedRW : IsConnected() && kNDASUnitConfigurationMountRW != Configuration()\n"));
					newStatus = kNDASUnitStatusTryDisconnectRW;
				}
			}
				break;
	...
	if (prevStatus != newStatus) {
		DbgIOLog(DEBUG_MASK_NDAS_WARNING, ("updateStatus: Change Status from %d to %d\n", prevStatus, newStatus));
...
UInt32	com_ximeta_driver_NDASPhysicalUnitDevice::blocksize()
{
	return SECTOR_SIZE;
}
