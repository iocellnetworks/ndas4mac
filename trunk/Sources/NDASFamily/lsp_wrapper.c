...
transfer_context(lsp_wrapper_context_t *context)
{
	...
	DbgIOLog(dbgTrace, ("[%s] %p, socket(%x)\n", __FUNCTION__, context, context->s));
	...
	for(;; context->lsp_status = lsp_process_next(context->lsp_handle))
	{
		if (LSP_REQUIRES_SEND == context->lsp_status)
		{
			...
			DbgIOLog(dbgTrace, ("[%d] send(%d, %p, %d)\n", ++sc, context->s, buffer, len_buffer));
		...
		else if (LSP_REQUIRES_RECEIVE == context->lsp_status)
		{
			...
			DbgIOLog(dbgTrace, ("[%d] recv(%d, %p, %d)\n", ++sc, context->s, buffer, len_buffer));
			{
				int res;
				char *buf = buffer;
				int len = len_buffer;
						...
						int timeoutsec)
{
	...
	DbgIOLog(DEBUG_MASK_LS_TRACE, ("xi_lpx_connect : socket(%x)\n", context->s));
					  ...
					  )
{
	...
	DbgIOLog(DEBUG_MASK_LS_TRACE, ("[%s] %p socket(%x)\n", __FUNCTION__, context, context->s));
	...
	DbgIOLog(DEBUG_MASK_LS_TRACE, ("lsp_handle(%x).\n", context->lsp_handle));
					   ...
					   )
{
	...
	if (lsp_wrapper_isconnected(context)) {
							 ...
							 TARGET_DATA *target_info)
{
	...
	*nr_target = list.number_of_elements;
    ...
    for(i = 0; i < *nr_target; i++) {
		...
		DbgIOLog(DEBUG_MASK_LS_INFO, ("[%s] target_info[%d](%d, %d, %d, %02X %02X %02X %02X %02X %02X %02X %02X)\n",
									  __FUNCTION__,
									  i,
