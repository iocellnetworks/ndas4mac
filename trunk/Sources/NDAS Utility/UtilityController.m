...
- (void) refreshNDASDevices
{
	...
	for (count = 0; count < numberOfObjects; count++) {
		...
		NDASInfo.slotNumber = indexArray[count];
...
			// Refresh 
	...
	while ((device = [enumerator nextObject])) {
		if ([device isRemoved]) {
			...
			[NDASDevices removeObjectForKey: [NSNumber numberWithInt: [device slotNo]]];
		}
	}
	...
	numberOfObjects =  NDASGetInformationGetCount(NDAS_RAID_DEVICE_CLASS_NAME, indexArray);
	
	for (count = 0; count < numberOfObjects; count++) {
		
		RAIDInfo.index = indexArray[count];
...
//			[tabView removeTabViewItem: singleUnitTab];
			...
			[backgroundInformationField setStringValue: [NSString stringWithFormat: NSLocalizedString(@"TAB_BACKGROUND : Select a NDAS device, or register a new one.", @"Instruction message. This text appears in the place of tab view when no deivce selected like 'Disk Utility'")]];
		}
...
//	NSLog(@"notification");
	...
	if ( nil != reply ) {
		...
		nsnSenderID = [reply objectForKey:[NSString stringWithCString: NDAS_NOTIFICATION_REPLY_SENDER_ID_KEY_STRING]];
		if ( nil != nsnSenderID ) {
			senderID = [nsnSenderID unsignedIntValue];
		}
				...
				switch ( operation ) {
					case NDAS_NOTIFICATION_REQUEST_REGISTER:
					{
						switch( errorCode ) {
							...
							{
								NSRunAlertPanel(
												[NSString stringWithFormat: NSLocalizedString(@"PANEL-TITLE : This NDAS device was registered Already.", @"Info Already Registered")],
												[NSString stringWithFormat: NSLocalizedString(@"PANEL-MSG : If you want to see the ID of registered NDAS device, Select the NDAS device at the NDAS device list.", @"Info Already Registered")],
	...
	if ( ! editingName && ! sentRequest ) {
		...
		if ( -1 != row) {
			[ndasOutlineView selectRow: row byExtendingSelection: NO];
		} else {
	...
	if(-1 < selectedRow) {
		...
		if ( [ndasDevice writable] ) {
			...
			int answer = NSRunCriticalAlertPanel(
												 ...
												 [NSString stringWithFormat: NSLocalizedString(@"PANEL-MSG : You may add the write key again at a later time.", @"Confirm removing write key")],
					 ...
					 Sync : (bool) sync
{
	...
	[mdCommand setObject: [NSNumber numberWithInt:IDForNotification] forKey: @NDAS_NOTIFICATION_SENDER_ID_KEY_STRING];
					...
					case kNDASDeviceTypeNDAS:
					{
						...
						/*
						 if ( kNDASStatusOnline == [(NDASDevice *)item status] ) { 
							 ...
							 image = [NSImage imageNamed:@"NDAS Failure"];
						 }
						 */
						[image setScalesWhenResized: TRUE];
						[image setSize: imageSize];
					...
					case kNDASStatusOnline:
						...
						if ( 0 < [device numberOfUnits] ) {
							...
							for(unitCount = 0; unitCount < MAX_NR_OF_TARGETS_PER_DEVICE; unitCount++) {
								...
								if ( unitDevice = [device unitAtIndex: unitCount]) {
									switch( [unitDevice status] ) {
										case kNDASUnitStatusConnectedRO:
										case kNDASUnitStatusConnectedRW:
...
- (IBAction)RAIDBindSheetBind:(id)sender
{
	...
	for ( index = 0; index < [RAIDBindSheetSubUnitDevices count]; index++) {
		...
		if (NDASGetLogicalDeviceInformationByLowerUnitDeviceID(&LDInfo)) {
			[logicalDevices insertObject: [[[NSNumber alloc] initWithInt: LDInfo.index] autorelease] atIndex: index];
		} else {
			return;
		}
	}
	...
	if(-1 < selectedRow) {
		...
		if ( kNDASUnitConfigurationUnmount == configuration
			...
			answer = NSRunCriticalAlertPanel(
									 ...
									 [NSString stringWithFormat: NSLocalizedString(@"PANEL-MSG : You may lose or corrupt your data.", @"Confirm unbinding")],
									 NSLocalizedString(@"PANEL-BUTTON : Cancel", @"Text for button in panel"),
			...
			switch([raidDevice unitDeviceType]) {
				...
				case NMT_RAID1R2:
				{
					...
					answer = NSRunAlertPanel(
											 ...
											 [NSString stringWithFormat: NSLocalizedString(@"PANEL-MSG : If you want to use the RAID device that was configured to old type, you should migrate that RAID Device.", @"Confirm migration")],
											 NSLocalizedString(@"PANEL-BUTTON : Cancel", @"Text for button in panel"),
