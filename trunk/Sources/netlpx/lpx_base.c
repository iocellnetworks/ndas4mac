...
void lpx_input(mbuf_t packet, int len)
{
	...
	if ( 0 != (error = mbuf_pullup( &packet, sizeof(struct lpxhdr))) ) {
		DEBUG_PRINT(DEBUG_MASK_LPX_ERROR, ("lpx_input: pullup failed. error %d\n", error));		
		return;
	}
	...
	switch (lpxHeader->pu.p.type) {
		case LPX_TYPE_DATAGRAM:
		{
			...
			lpx_datagram_input((struct mbuf *)packet, lpxp, etherHeader);
				...
				struct lpxpcb *lpxp)
{
    ...
    DEBUG_PRINT(DEBUG_MASK_LPX_TRACE, ("lpx_output: Entered.\n"));
    ...
    if (ntohs(lpxh->pu.pktsize & ~LPX_TYPE_MASK) <= ifnet_mtu(interface)) {
		...
		if (error != 0) {
			DEBUG_PRINT(DEBUG_MASK_LPX_ERROR, ("lpx_output: lpx_ifnet_output error %d\n"));
...
struct lpx_ifaddr * lpx_iaonnetof( register struct lpx_addr *dst )
{
    ...
    DEBUG_PRINT(DEBUG_MASK_LPX_TRACE, ("lpx_iaonnetof: Entered. dst->x_net = %x\n", dst->x_net));
