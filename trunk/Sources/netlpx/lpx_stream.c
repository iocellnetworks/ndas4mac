...
*/
    ...
    DEBUG_PRINT(DEBUG_MASK_STREAM_INFO, ("lpx_stream_input: lpxh->dest_port = %x, lpxh->u.s.sequence = %x, lpxh->u.s.ackseq = %x, cb->s_rack = %x, cb->s_seq = %x, cb->s_ack = %x\n", ntohs(lpxh->dest_port), lpxh->u.s.sequence, lpxh->u.s.ackseq, cb->s_rack, cb->s_seq, cb->s_ack));
    DEBUG_PRINT(DEBUG_MASK_STREAM_INFO, ("lpx_stream_input: ntohs(lpxh->u.s.lsctl) = %x\n", ntohs(lpxh->u.s.lsctl)));
	
    so = lpxp->lpxp_socket;
		
    DEBUG_PRINT(DEBUG_MASK_STREAM_INFO, ("lpx_stream_input : to so@%lx\n",so));
...
//			sock_retain((socket_t)so);
    ...
    if (ntohs(lpxh->u.s.lsctl) & LSCTL_ACKREQ)
        cb->s_flags |= (lpx_stream_use_delack ? SF_DELACK : SF_ACKNOW);
...
//    int s = splnet();
    ...
    DEBUG_PRINT(DEBUG_MASK_STREAM_TRACE, ("lpx_stream_user_attach: Entered. so@%lx: lpxp@%lx\n",so,lpxp));
                            ...
                            struct proc *td )
{
 ...
 //   int s;
   ...
   //s = splnet();
     ...
     */
    lpxp->lpxp_fport = 0;
    DEBUG_PRINT(DEBUG_MASK_STREAM_INFO, ("lpx_stream_usr_connect for lpxp@%lx, lpxp->lpxp_laddr.x_port = %x\n",
...
//    int s;
 ...
 //   s = splnet();
	...
	if (error) {
        DEBUG_PRINT(DEBUG_MASK_STREAM_ERROR,("lpx_stream_usr_send: error(%d) in lpx_stream_output so@%lx, cb@%lx, m@%lx\n",error,so,cb,m));
    }
    m = NULL;
lpx_stream_send_end:
    if (controlp != NULL)
...
//    int s = splnet();
    ...
    if (cb == 0)
        goto out;
    cb = lpx_stream_drop(cb, ECONNABORTED);
...
static int lpx_stream_attach( struct socket *so, struct proc *td )
{
    ...
    DEBUG_PRINT(DEBUG_MASK_STREAM_INFO, ("lpx_stream_attach with so@%lx: ==> lpxp@%lx : pcb@%lx\n",so,lpxp,lpxp->lpxp_pcb));
...
int lpx_stream_output(register struct stream_pcb *cb, struct mbuf *m0)
{
    ...
    DEBUG_PRINT(DEBUG_MASK_STREAM_TRACE, ("lpx_stream_output: Entered. so@%lx pcb@%lx\n",so, cb));
...
send:
    ...
    if (lpxh != NULL) {
        ...
        switch (cb->s_state) {
			...
			case LPX_STREAM_LAST_ACK:
			{
				DEBUG_PRINT(DEBUG_MASK_STREAM_INFO, ("lpx_stream_output: LPX_STREAM_FIN_WAIT_1 send\n"));
			...
			default:
			{
				DEBUG_PRINT(DEBUG_MASK_STREAM_INFO, ("lpx_stream_output: default\n"));
	...
	
    {
         ...
         */		
        DEBUG_PRINT(DEBUG_MASK_STREAM_INFO, ("cb->s_state = %x, sequence = %x, ackseq = %x\n", cb->s_state, lpxh->u.s.sequence, lpxh->u.s.ackseq));
        ...
        if (so && so->so_options & SO_DONTROUTE) {
            error = lpx_output(m, LPX_ROUTETOIF, lpxp);
            if (error) {
				DEBUG_PRINT(DEBUG_MASK_STREAM_ERROR, ("lpx_stream_output: error(%d) in Lpx_OUT_outputfl m@%lx, lpxp@%lx\n",error,m,lpxp));
            }
        } else {
            error = lpx_output(m, 0, lpxp);
            if (error) {
				DEBUG_PRINT(DEBUG_MASK_STREAM_ERROR, ("lpx_stream_output: error(%d) in Lpx_OUT_outputfl m@%lx\n",error,m));
            }
        }
    }
...
struct stream_pcb *lpx_stream_timers(register struct stream_pcb *cb, int timer)
{
    ...
    switch (timer) {
		...
		case SMPT_2MSL:
		{
			...
			DEBUG_PRINT(DEBUG_MASK_TIMER_INFO, ("lpx_stream_timers: SMPT_2MSL went off for no reason flag 0x%x\n", cb->s_flags_control));
		...
		case SMPT_REXMT:
		{
			...
			DEBUG_PRINT(DEBUG_MASK_TIMER_INFO, ("lpx_stream_timers: SMPT_REXMT went off for no reason, cb->s_rxtshift = %d, SMP_MAXRXTSHIFT = %d\n", cb->s_rxtshift, SMP_MAXRXTSHIFT));
			...
			if (++cb->s_rxtshift > maxRTshift) {
				...
				if (maxRTshift == SMP_MAXRXTSHIFT) {
					DEBUG_PRINT(DEBUG_MASK_TIMER_ERROR, ("lpx_stream_timer: End Retransmit. cb 0x%x rxtshift %d\n", cb, cb->s_rxtshift));
				}
		...
		case SMPT_KEEP:
		{
			...
			if (cb->s_state < LPX_STREAM_ESTABLISHED) {
				...
				DEBUG_PRINT(DEBUG_MASK_TIMER_INFO, ("lpx_stream_timers: Keep alive. Not connected. cb 0x%x, status %d\n", cb, cb->s_state));
			...
			if (cb->s_lpxpcb->lpxp_socket && cb->s_lpxpcb->lpxp_socket->so_options & SO_KEEPALIVE) {
				...
				if (cb->s_idle >= SMPTV_MAXIDLE) {
					DEBUG_PRINT(DEBUG_MASK_TIMER_ERROR, ("lpx_stream_timers: End Keep alive. cb->s_idle = %d, SMPTV_MAXIDLE = %d\n", cb->s_idle, SMPTV_MAXIDLE));
...
lpx_stream_lock(struct socket *so, int refcount, int lr) {
	...
	if (so->so_pcb) {
		...
		panic("lpx_stream_lock: so=%x NO PCB!\n", so);
		lck_mtx_assert(so->so_proto->pr_domain->dom_mtx, LCK_MTX_ASSERT_NOTOWNED);
		lck_mtx_lock(so->so_proto->pr_domain->dom_mtx);
	}
	
	if (so->so_usecount < 0)
		panic("lpx_stream_lock: so=%x so_pcb=%x ref=%x\n",
...
lpx_stream_unlock(struct socket *so, int refcount, int lr) {
	...
	if (so->so_usecount < 0)
		panic("lpx_stream_unlock: so=%x usecount=%x\n", so, so->so_usecount);	
	if (so->so_pcb == NULL) {
		panic("lpx_stream_unlock: so=%x NO PCB usecount=%x\n", so, so->so_usecount);
		lck_mtx_assert(so->so_proto->pr_domain->dom_mtx, LCK_MTX_ASSERT_OWNED);	
		lck_mtx_unlock(so->so_proto->pr_domain->dom_mtx);
	}
	else {
...
lpx_stream_getlock(struct socket *so, int locktype) {
	...
	if (so->so_pcb)  {
		if (so->so_usecount < 0)
			panic("lpx_stream_getlock: so=%x usecount=%x\n", so, so->so_usecount);	
		return(lpxp->lpxp_mtx);
	}
	else {
		panic("lpx_stream_getlock: so=%x NULL so_pcb\n", so);
