 ...
 */
void lpx_datagram_input(mbuf_t m,
						register struct lpxpcb *lpxp,
						struct ether_header *frame_header)
{
    register struct lpxhdr *lpxh = mbuf_data(m);
...
*/	
	...
	*/
	...
	// Remove Lpx Header. 
	mbuf_adj(m, sizeof(struct lpxhdr));
  
	// Drop Bad Packet.
	if (mbuf_len(m) < ntohs(lpxh->u.d.message_length)) {
		goto bad;
	}
	
	// Trim data.
	if (mbuf_len(m) > ntohs(lpxh->u.d.message_length)) {
		mbuf_adj(m, - (mbuf_len(m) - ntohs(lpxh->u.d.message_length)));
	}

	if (sbappendaddr(&lpxp->lpxp_socket->so_rcv, (struct sockaddr *)&lpx_lpx,
                     (struct mbuf *)m, (struct mbuf *)NULL, &error) == 0) {
        DEBUG_PRINT(DEBUG_MASK_DGRAM_WARING, ("Lpx_USER_input: Maybe No space for this packet. sbappendaddr error: socket=@(%p), rcv@(%p)!",lpxp->lpxp_socket, &(lpxp->lpxp_socket->so_rcv)));
		...
		DEBUG_PRINT(DEBUG_MASK_DGRAM_INFO, ("Lpx_USER_input: wokeup socket @(%p)\n",lpxp->lpxp_socket));
	}
...
//	int s;
	...
	if (lpxp == 0)
		panic("lpx_datagram_user_abort: so %p null lpxpcb\n", so);
...
//    int s;
	...
	if (lpxp != 0)
		panic("lpx_datagram_user_attach: so %p lpxpcb=%p\n", so, lpxp);
...
//        s = splnet();
        ...
        if (error) {
//            splx(s);
            DEBUG_PRINT(DEBUG_MASK_DGRAM_ERROR, ("lpx_USER_send: error(%d) in Lpx_PCB_connect: so=%p, lpxp=%p, nam=%p\n",error,so, lpxp, nam));
            goto send_release;
        }
    } else {		
        if (lpx_nullhost(lpxp->lpxp_faddr)) {
            error = ENOTCONN;
            DEBUG_PRINT(DEBUG_MASK_DGRAM_ERROR, ("lpx_USER_send: error(%d): so=%p, lpxp=%p\n",error,so, lpxp ));
            goto send_release;
        }
    }
    error = lpx_datagram_user_output(lpxp, m);
    if (error) {
		DEBUG_PRINT(DEBUG_MASK_DGRAM_ERROR, ("lpx_USER_send: error(%d) in lpx_USER_output: so=%p, lpxp=%p, m=%p\n",error, so, lpxp, m));
    }
    m = NULL;
    if (nam != NULL) {
        Lpx_PCB_disconnect(lpxp);
                            ...
                            struct mbuf *m0 )
{
    ...
    return (lpx_output((mbuf_t)m, so->so_options & SO_BROADCAST, lpxp));
}
...
int lpx_datagram_lock(struct socket *so, int refcount, int debug) {
	...
	if (so->so_pcb) {
		...
		panic("lpx_datagram_lock: so=%p NO PCB!\n", so);
		lck_mtx_assert(so->so_proto->pr_domain->dom_mtx, LCK_MTX_ASSERT_NOTOWNED);
		lck_mtx_lock(so->so_proto->pr_domain->dom_mtx);
	}
...
int lpx_datagram_unlock(struct socket *so, int refcount, int debug) {
	...
	if (so->so_pcb == NULL) {
		panic("lpx_datagram_unlock: so=%p NO PCB!\n", so);
		lck_mtx_assert(so->so_proto->pr_domain->dom_mtx, LCK_MTX_ASSERT_OWNED);
		lck_mtx_unlock(so->so_proto->pr_domain->dom_mtx);
	}
	else {
