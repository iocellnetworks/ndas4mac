									 ...
									 mbuf_t packet, char* header)
{
	...
	// Check minimum size.
	if (mbuf_pkthdr_len(packet) < sizeof(struct lpxhdr)) {
		DEBUG_PRINT(DEBUG_MASK_IF_ERROR, ("lpx_ifnet_input: Too small packet size %d\n", (uint32_t)mbuf_pkthdr_len(packet)));
	...
	// Pullup Header data.
	if ((mbuf_flags(packet) & M_EXT || (mbuf_len(packet) < sizeof(struct lpxhdr))) 
	...
	//
	if (mbuf_pkthdr_len(packet) > packetSize) {
		DEBUG_PRINT(DEBUG_MASK_IF_INFO, ("lpx_ifnet_input: Total size %d, m size %d, real size %d\n", (uint32_t)mbuf_pkthdr_len(packet), (uint32_t)mbuf_len(packet), packetSize));
		
		mbuf_adj(packet, - (mbuf_pkthdr_len(packet) - packetSize));
	}
