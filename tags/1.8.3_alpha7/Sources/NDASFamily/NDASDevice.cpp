...
void com_ximeta_driver_NDASDevice::setStatus(uint32_t status)
{
	if(fStatus != status) {
		...
		if (NULL == bus) {
			...
			bus->messageClients(kNDASFamilyIOMessageNDASDeviceStatusWasChanged, &fSlotNumber, sizeof(uint32_t));
		}
	}
}
													  ...
													  )
{
	...
	pScsiCommand->MemoryDescriptor_ptr->prepare();
...
/*		
	...
	pScsiCommand->MemoryDescriptor_ptr->complete();
