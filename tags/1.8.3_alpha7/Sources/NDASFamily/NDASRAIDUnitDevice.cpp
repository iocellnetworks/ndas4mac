...
void	com_ximeta_driver_NDASRAIDUnitDevice::setType(uint32_t type) 
{
	fType = type;
...
uint32_t com_ximeta_driver_NDASRAIDUnitDevice::Type()
{
	return fType;
}

void	com_ximeta_driver_NDASRAIDUnitDevice::setIndex(uint32_t index)
{
	fIndex = index;
...
uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::Index()
{
	return fIndex;
}
...
void	com_ximeta_driver_NDASRAIDUnitDevice::setRMDUSN(uint32_t usn)
{
	fRMDUSN = usn;
...
uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::RMDUSN()
{
	return fRMDUSN;
}
...
uint64_t	com_ximeta_driver_NDASRAIDUnitDevice::getNumberOfSectors()
{
...
uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::getNRRWHosts()
{
	uint32_t hosts = 0;
...
uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::getNRROHosts()
{
	uint32_t hosts = 0;
...
void	com_ximeta_driver_NDASRAIDUnitDevice::setConfigurationOfSubUnits(uint32_t configuration)
{
...
void	com_ximeta_driver_NDASRAIDUnitDevice::setConfiguration(uint32_t configuration)
{
...
bool com_ximeta_driver_NDASRAIDUnitDevice::readSectorsOnce(uint64_t location, uint32_t sectors, char *buffer)
{
	uint64_t	endSector;
	int		count;
	...
	switch(fType) {
		...
		{
			uint64_t	endSector;
			uint64_t	offset, reqSectors, reqLocation;
...
bool com_ximeta_driver_NDASRAIDUnitDevice::writeSectorsOnce(uint64_t location, uint32_t sectors, char *buffer)
{
	int count;
...
bool com_ximeta_driver_NDASRAIDUnitDevice::processSRBCommand(com_ximeta_driver_NDASSCSICommand *command)
{
...
void com_ximeta_driver_NDASRAIDUnitDevice::completeRecievedCommand()
{
	...
	com_ximeta_driver_NDASSCSICommand *receivedCommand = NDASCommand();
...
void com_ximeta_driver_NDASRAIDUnitDevice::completeCommand(com_ximeta_driver_NDASSCSICommand *command)
{	
...
bool	com_ximeta_driver_NDASRAIDUnitDevice::setUnitDevice(int32_t index, com_ximeta_driver_NDASUnitDevice *device)
{
	DbgIOLog(DEBUG_MASK_RAID_ERROR, ("setUnitDevice: Entered. device [%d] %p\n", index, device));
	...
	if (device) {
		...
		mySnprintf(buffer, sizeof(buffer), NDASDEVICE_PROPERTY_KEY_RAID_UNIT_DISK_LOCATION_OF_SUB_UNIT, index);
		
		memcpy(&temp, device->UnitDiskLocation(), sizeof(uint64_t));
		
		setProperty(buffer, temp, 64);		
		
		// Set SubUnit Type.
		uint32_t SubUnitType = kNDASRAIDSubUnitTypeNormal; 
...
void	com_ximeta_driver_NDASRAIDUnitDevice::setCountOfUnitDevices(uint32_t count)
{
...
uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::CountOfUnitDevices()
{
...
uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::CountOfRegistetedUnitDevices()
{
	DbgIOLog(DEBUG_MASK_RAID_TRACE, ("CountOfRegistetedUnitDevices: Entered.\n"));

	uint32_t countOfUnits = 0;
...
void	com_ximeta_driver_NDASRAIDUnitDevice::setCountOfSpareDevices(uint32_t count)
{
...
uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::CountOfSpareDevices()
{
...
void	com_ximeta_driver_NDASRAIDUnitDevice::setSenseKey(uint32_t senseKey)
{
	fSenseKey = senseKey;
}

uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::senseKey()
{
	return fSenseKey;
}

com_ximeta_driver_NDASUnitDevice	*com_ximeta_driver_NDASRAIDUnitDevice::UnitDeviceAtIndex(uint32_t index)
{
...
void	com_ximeta_driver_NDASRAIDUnitDevice::setNDASCommand(com_ximeta_driver_NDASSCSICommand *command)
{
	if(fNDASCommand) {
...
com_ximeta_driver_NDASSCSICommand *com_ximeta_driver_NDASRAIDUnitDevice::NDASCommand()
{
	return fNDASCommand;
}

uint32_t com_ximeta_driver_NDASRAIDUnitDevice::maxRequestSectors()
{
				...
				case NMT_RAID0R2:
				{
					...
					for (int count = 1; count < 10; count++) {
						if(!writeSectorsOnce(count, 1, buffer)) {
							DbgIOLog(DEBUG_MASK_RAID_ERROR, ("unbind: Can't Write Apple Partition Info Location %d\n", count));
						}		
					}
				}
					break;
...
bool	com_ximeta_driver_NDASRAIDUnitDevice::rebind()
{
	...
	if (kNDASRAIDStatusGoodWithReplacedUnit == RAIDStatus()
		...
		for(count = 0; count < CountOfUnitDevices() + CountOfSpareDevices(); count++) {
			...
			uint32_t index = getRMDIndexOfUnit(&rmd, count);
			...
			if (count != index
				...
				for (innerCount = 0; innerCount <  CountOfUnitDevices() + CountOfSpareDevices(); innerCount++) {
					...
					if (UnitDeviceAtIndex(innerCount)) {
						...
						// DIB
						memcpy(&UnitDeviceAtIndex(innerCount)->DIB2()->UnitDisks[index], UnitDeviceAtIndex(index)->UnitDiskLocation(), sizeof(uint64_t));
...
bool	com_ximeta_driver_NDASRAIDUnitDevice::convertBindType(uint32_t newType)
{
	uint64_t					location;
	char					buffer[512] = { 0 };
				...
				for (count = 0; count < CountOfUnitDevices(); count++) {
					for (int sectorPointer = 0;  sectorPointer < NDAS_BLOCK_SIZE_BITMAP_REV1; sectorPointer++) {
						...
						if (UnitDeviceAtIndex(count)->readSectorsOnce(location, 1, buffer)) {
							...
							DbgIOLog(DEBUG_MASK_RAID_INFO, ("convertBindType: RAID1 Can't Read bitmap sequence %d, sectorOffset %d.\n", count, sectorPointer));						
						}
					}
				}
			}
...
void	com_ximeta_driver_NDASRAIDUnitDevice::setUnitDevices(PNDAS_DIB_V2 dib2, PNDAS_RAID_META_DATA rmd)
{
	...
	for (count = 0; count < (getDIBCountOfUnitDisks(dib2) + getDIBCountOfSpareDevices(dib2)); count++) {
		...
		mySnprintf(buffer, sizeof(buffer), NDASDEVICE_PROPERTY_KEY_RAID_UNIT_DISK_LOCATION_OF_SUB_UNIT, count);
		...
		memcpy(&temp, getDIBUnitDiskLocation(dib2, index), sizeof(uint64_t));
...
void	com_ximeta_driver_NDASRAIDUnitDevice::setStatusOfSubUnit(uint32_t index, uint32_t status)
{
	char	buffer[256];
	...
	if (index < CountOfUnitDevices() + CountOfSpareDevices()) {
		...
		mySnprintf(buffer, sizeof(buffer), NDASDEVICE_PROPERTY_KEY_RAID_STATUS_OF_SUB_UNIT, index);
		
		setProperty(buffer, fSubUnitStatus[index], 32);	
	}	
}

uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::StatusOfSubUnit(uint32_t index) 
{
	if (index < CountOfUnitDevices() + CountOfSpareDevices()) {
...
void	com_ximeta_driver_NDASRAIDUnitDevice::setTypeOfSubUnit(uint32_t index, uint32_t type)
{
	char	buffer[256];
	...
	if (index < CountOfUnitDevices() + CountOfSpareDevices()) {
		...
		mySnprintf(buffer, sizeof(buffer), NDASDEVICE_PROPERTY_KEY_RAID_TYPE_OF_SUB_UNIT, index);
		
		setProperty(buffer, fSubUnitType[index], 32);	
	}		
}

SInt32	com_ximeta_driver_NDASRAIDUnitDevice::TypeOfSubUnit(uint32_t index)
{
	if (index < CountOfUnitDevices() + CountOfSpareDevices()) {
...
void	com_ximeta_driver_NDASRAIDUnitDevice::setRAIDStatus(uint32_t status)
{
...
uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::RAIDStatus()
{
	return fRAIDStatus;
}

uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::RAIDBlocksPerSector()
{
	return  blockSize() / SECTOR_SIZE;
}

bool	com_ximeta_driver_NDASRAIDUnitDevice::IsDirtyBitSet(uint64_t sector, bool *dirty)
{
	...
	uint32_t	sectorsPerBit = getDIBSectorsPerBit(GoodSubUnit->DIB2());
	UInt8	BitMapBitPoint = (sector / sectorsPerBit) % 8;
	uint32_t	BitMapSectorPoint = (sector / sectorsPerBit) / NDAS_BIT_PER_OOS_BITMAP_BLOCK; 
	uint32_t	BitMapBytePoint = (sector / sectorsPerBit / 8) % NDAS_BYTE_PER_OOS_BITMAP_BLOCK; 
...
bool com_ximeta_driver_NDASRAIDUnitDevice::SetBitmap(uint64_t location, uint32_t sectors, bool dirty)
{
	if (sectors == 0) {
		return false;
	}
...
bool com_ximeta_driver_NDASRAIDUnitDevice::ClearBitmapFromFirstSector(uint64_t location) 
{
	return UpdateBitMapBit(0, location + 1, false);
}


bool	com_ximeta_driver_NDASRAIDUnitDevice::UpdateBitMapBit(uint64_t location, uint32_t sectors, bool dirty)
{
	...
	uint64_t	nomalizedLocation;

	uint64_t	dirtyBitSector;
	...
	while (nomalizedLocation <= (location + sectors - 1)) {
		...
		uint32_t	BitMapSectorPoint = (nomalizedLocation / sectorsPerBit) / NDAS_BIT_PER_OOS_BITMAP_BLOCK; 
		uint32_t	BitMapBytePoint = (nomalizedLocation / sectorsPerBit / 8) % NDAS_BYTE_PER_OOS_BITMAP_BLOCK; 
		bool	bPrevious;
...
void	com_ximeta_driver_NDASRAIDUnitDevice::recovery()
{
	...
	if (!fInRecoveryProcess) {
		...
		if (NULL == fDirtyBitmap) {
			...
			if (false == allocDirtyBitmap()) {
				...
				if (GoodSubUnit != NULL) {
					...
					uint64_t	dirtyBitSector = GoodSubUnit->NumberOfSectors() + NDAS_BLOCK_LOCATION_BITMAP;
		...
		// Update RMD.
		uint64_t location = BadSubUnit->NumberOfSectors() + NDAS_BLOCK_LOCATION_RMD;
	...
	switch(Type()) {
		...
		{			
			...
			if ( IsDirtyBitSet(fSectorOfRecovery, &dirty) && dirty) {
				...
				uint32_t	sectorsForRecovery = min(maxRequestBlocks, Capacity() - fSectorOfRecovery);
...
uint32_t	com_ximeta_driver_NDASRAIDUnitDevice::blockSize()
{
	uint32_t	size;
...
{
	uint32_t	prevRAIDStatus = 0;
	uint32_t	newRAIDStatus = 0;
	int		count;
	...
	for (count = 0; count < fCountOfUnitDevices + fCountOfSpareDevices; count++) {
		...
		uint32_t	newUnitStatus = StatusOfSubUnit(count);
...
uint32_t com_ximeta_driver_NDASRAIDUnitDevice::updateStatus()
{
	DbgIOLog(DEBUG_MASK_RAID_TRACE, ("getStatus: updateStatus.\n"));
	
	uint32_t	newStatus;	// Not assigned.
	uint32_t	prevStatus;
	int count;
...
bool com_ximeta_driver_NDASRAIDUnitDevice::postProcessingforConnectRW() {
	...
	for (count = 0;  count < CountOfUnitDevices() + CountOfSpareDevices(); count++) {
		...
		if (unitDevice) {
			...
			uint64_t location = unitDevice->NumberOfSectors() + NDAS_BLOCK_LOCATION_RMD;
...
void com_ximeta_driver_NDASRAIDUnitDevice::completeChangeSettingMessage(
															  com_ximeta_driver_NDASChangeSettingCommand *command
															  )
{
	if ( NULL != fPnpCommand ) {
...
bool	com_ximeta_driver_NDASRAIDUnitDevice::ReplaceToSpare()
{
	...
	for (int count = 0; count < CountOfUnitDevices() + CountOfSpareDevices(); count++) {
		...
		if (UnitDeviceAtIndex(count)
			...
			// Update RMD.
			uint64_t location = UnitDeviceAtIndex(count)->NumberOfSectors() + NDAS_BLOCK_LOCATION_RMD;
	...
	mySnprintf(buffer, sizeof(buffer), NDASDEVICE_PROPERTY_KEY_RAID_UNIT_DISK_LOCATION_OF_SUB_UNIT, indexOfSpareSubUnit);
	
	memcpy(&temp, getDIBUnitDiskLocation(UnitDeviceAtIndex(indexOfGoodSubUnit)->DIB2(), indexOfBrokenSubUnit), sizeof(uint64_t));
