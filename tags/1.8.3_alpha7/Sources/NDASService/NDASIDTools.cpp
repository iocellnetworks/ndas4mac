...
checkSNValid(char *strID1, char *strID2, char *strID3, char *strID4, char *wKey, char *tempMac, bool *bWriteKey, uint8_t *vid)
{
	...
	if((ret = DecryptSerial(&info)) == TRUE &&
	   ...
	   (strlen(wKey) == 0 || (strlen(wKey) == 5 && info.bIsReadWrite == TRUE))) {
		...
		if (NDAS_ID_EXTENSION_DEFAULT.VID == info.ucVid
			&& NDAS_ID_EXTENSION_DEFAULT.Seed == info.ucRandom
			&& NDAS_ID_EXTENSION_DEFAULT.Reserved[0] == info.reserved[0]
			&& NDAS_ID_EXTENSION_DEFAULT.Reserved[1] == info.reserved[1]) {
			// Default ID.
			...
			// Seagate ID.
		}  else if (NDAS_ID_EXTENSION_SOFT_NETDISK.VID == info.ucVid
					&& NDAS_ID_EXTENSION_SOFT_NETDISK.Seed == info.ucRandom
					&& NDAS_ID_EXTENSION_SOFT_NETDISK.Reserved[0] == info.reserved[0]
					&& NDAS_ID_EXTENSION_SOFT_NETDISK.Reserved[1] == info.reserved[1]) {
			// Soft Netdisk ID.
		}  else {
			return FALSE;
		}
