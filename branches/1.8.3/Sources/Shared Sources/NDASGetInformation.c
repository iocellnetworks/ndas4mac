...
bool NDASGetInformationBySlotNumber(PNDASPhysicalDeviceInformation parameter) 
{
	...
	CFBooleanRef	cfbAutoRegister = NULL;
	...
	if(cfdElement != NULL) {
		...
		// Auto Register.
		if(CFDictionaryGetValueIfPresent(cfdElement, CFSTR(NDASDEVICE_PROPERTY_KEY_AUTOREGISTER), (const void**)&cfbAutoRegister)
		   && cfbAutoRegister) {
			CFRetain(cfbAutoRegister);
			parameter->autoRegister = CFBooleanGetValue(cfbAutoRegister);
		}
...
cleanup:
	...
	if(cfbAutoRegister)		CFRelease(cfbAutoRegister);
	...
	if (numberOfDevices > 0) {
		...
		while(deviceUpNext) {
			...
			if (KERN_SUCCESS == kernResult
				...
				// SlotNumber is Key.
				if (CFDictionaryGetValueIfPresent(properties, CFSTR(NDASDEVICE_PROPERTY_KEY_SLOT_NUMBER), (const void**)&cfnValue)) {
					CFNumberGetValue(cfnValue, kCFNumberSInt32Type, &value);
