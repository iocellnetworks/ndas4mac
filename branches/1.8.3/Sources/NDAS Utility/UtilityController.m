...
//	NSLog(@"refreshNDASDevices\n");
	...
	indexArray = (NSMutableArray *)NDASGetInformationGetIndexArray(NDAS_DEVICE_CLASS_NAME);
...
			// Refresh 
	...
	indexArray = (NSMutableArray *)NDASGetInformationGetIndexArray(NDAS_RAID_DEVICE_CLASS_NAME);
...
//	[nameField setEditable: NO];
	...
	if (ndas) {
		...
		if(![ndas autoRegister]) {
			...
			if ( kNDASStatusOffline == [ndas status] ) {
				[unregisterMenuItem setEnabled: TRUE];
			} else if ( kNDASStatusOnline == [ndas status] ) {
 ...
 */
		...
		int answer = NSRunCriticalAlertPanel(
											 ...
											 [NSString stringWithFormat: NSLocalizedString(@"PANEL-MSG : If you want to register '%@'(NDAS device name) to another computer after key registered, You should enter the NDAS ID manualy.", @"Register key Note"), [ndEntry name]],
											 NSLocalizedString(@"PANEL-BUTTON : Cancel", @"Text for button in panel"),
				...
				switch ([device status]) {
					case kNDASStatusOnline:
						...
						if ( 0 < [device numberOfUnits] ) {
							...
							int unitCount;							
							int imageType = 0; // 0 = gray, 1 = green, 2 = yellow, 3 = red.
    ...
    if ([[toolbarItem itemIdentifier] isEqual: RegisterToolbarItemIdentifier]) {
		...
		if ( 0 <= [ndasOutlineView selectedRow] ) {
			...
			if ( kNDASDeviceTypeNDAS == [(Device *)device deviceType] ) {
				...
				if ( ![ndas autoRegister] || kNDASStatusOffline == [ndas status] ) {
					enable = YES;
				} else {
					enable = NO;
				}
