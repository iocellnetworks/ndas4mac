...
bool com_ximeta_driver_NDASBusEnumerator::init(OSDictionary *dict)
{
	...
	fCriticalSection_slock_grp_attr = lck_grp_attr_alloc_init();
	lck_grp_attr_setstat(fCriticalSection_slock_grp_attr);
	
	fCriticalSection_slock_grp =  lck_grp_alloc_init("com.ximeta.BusEnumertator.CriticalSection", fCriticalSection_slock_grp_attr);
	
	/* Allocate lock attribute */
	fCriticalSection_slock_attr = lck_attr_alloc_init();
	//lck_attr_setdebug(tz_slock_attr); // set the debug flag
	//lck_attr_setdefault(tz_slock_attr); // clear the debug flag
	
	/* Allocate the spin lock */
	fCriticalSectionMutex = lck_mtx_alloc_init(fCriticalSection_slock_grp, fCriticalSection_slock_attr);
#else 
	fCriticalSectionMutex = mutex_alloc(ETAP_NO_TRACE);
#endif
...
void com_ximeta_driver_NDASBusEnumerator::free(void)
{
	...
	// Destroy mutex
#if __ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ >= 1040
	lck_mtx_free(fCriticalSectionMutex, fCriticalSection_slock_grp);
	lck_attr_free(fCriticalSection_slock_attr);
	lck_grp_free(fCriticalSection_slock_grp);
	lck_grp_attr_free(fCriticalSection_slock_grp_attr);
#else
	mutex_free(fCriticalSectionMutex);
#endif
...
{
    DbgIOLog(DEBUG_MASK_USER_ERROR, ("--------------------Register---------------\n"));
	...
	if (!registerData->AutoRegister) {
		if ((registerData->VID & NDAS_VID_AUTO_MASK)) {
			DbgIOLog(DEBUG_MASK_USER_ERROR, ("Bad VID.\n"));
...
#if __ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ >= 1040
	lck_mtx_lock(fCriticalSectionMutex);
#else 
	mutex_lock(fCriticalSectionMutex);
#endif
	
	if (registerData->SlotNo == 0) {
		result->SlotNo = FindEmptySlotNumberOfNDASDevice(true);
	} else {
		if(FindNDASDeviceBySlotNumber(registerData->SlotNo) != NULL) {
			// Can't use that slot number.
			DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("Slot %d is already in use!!!\n", registerData->SlotNo));
						
			returnValue =  kIOReturnError;
	...
	if (result->SlotNo == 0) {
		DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("Can't Assign New SlotNumber!!!\n"));
	...
	// Check Address.
	NDASDevicePtr = FindNDASDeviceByAddress(&registerData->DevAddr);
	
	if (NDASDevicePtr) {
		
		DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("Found.\n"));
		
		// Check Status.
		switch(NDASDevicePtr->status()) {
			...
			{					
				// Assign attributes.
				NDASDevicePtr->setSlotNumber(result->SlotNo);
				NDASDevicePtr->setWriteRight( (registerData->hasWriteRight == 1 ? true : false ) );
				...
				NDASDevicePtr->setStatus(kNDASStatusOffline);
				NDASDevicePtr->setAutoRegister( (registerData->AutoRegister == 1 ? true : false) );
				
				returnValue =  kIOReturnSuccess;
			}
				break;
			default:
				DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("Already Registerd Device. status = %dn", NDASDevicePtr->status()));

				// Already registerted Device.
		...
		registerData->SlotNo = result->SlotNo;
		bret = createNDASDevice(registerData, kNDASStatusOffline);
		if (bret) {
						
			int32_t slotNo = result->SlotNo;
			messageClients(kNDASFamilyIOMessageNDASDeviceWasCreated, &slotNo, sizeof(uint32_t));
			
			returnValue =  kIOReturnSuccess;
			
		} else {
			DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("Can't create Device. status = %dn", NDASDevicePtr->status()));
...
#if __ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ >= 1040
	lck_mtx_unlock(fCriticalSectionMutex);
#else 
	mutex_unlock(fCriticalSectionMutex);
#endif
														 ...
														 IOByteCount			*outStructSize)
{
	...
	slotNo = NDASDevicePtr->slotNumber();
	
	//deleteNDASDevice(NDASDevicePtr);
	NDASDevicePtr->setSlotNumber(FindEmptySlotNumberOfNDASDevice(false));
	NDASDevicePtr->setStatus(kNDASStatusNotPresent);
	NDASDevicePtr->setConfiguration(kNDASConfigurationEnable);	// To receive PnP Message.
...
// kNDASEnumRegisterKey
IOReturn com_ximeta_driver_NDASBusEnumerator::RegisterKey(PNDAS_ENUM_REGISTER_KEY	registerKeyData,
														  uint32_t					*result,
														  IOByteCount				inStructSize,
														  IOByteCount				*outStructSize)
{
	...
	DbgIOLog(DEBUG_MASK_USER_ERROR, ("--------------------RegisterKey---------------\n"));
	...
	NDASDevicePtr = FindNDASDeviceBySlotNumber(registerKeyData->SlotNo);
	
	if(NDASDevicePtr == NULL) {
		DbgIOLog(DEBUG_MASK_USER_ERROR, ("Can't Find Registed Slot Number %d.\n", registerKeyData->SlotNo));
		
		return kIOReturnError;
	}
	
	if (kNDASStatusOnline != NDASDevicePtr->status() && kNDASStatusNeedRegisterKey != NDASDevicePtr->status()) {
		DbgIOLog(DEBUG_MASK_USER_ERROR, ("Bad Status %d.\n", NDASDevicePtr->status()));
		
		return kIOReturnError;		
	}
	
	NDASDevicePtr->receiveServiceRequest(
										 kNDASServiceCommandChangeUserPassword,
										 registerKeyData->userPassword,
										 registerKeyData->vid,
										 registerKeyData->bWritable
										 );
		
	*result = registerKeyData->SlotNo;
	*outStructSize = (IOByteCount)(sizeof(uint32_t));
...
void com_ximeta_driver_NDASBusEnumerator::releaseBroadcastReceiver(int interfaceArrayIndex)
{
	DbgIOLog(DEBUG_MASK_NDAS_TRACE, ("Entered. index : %d\n", interfaceArrayIndex));
...
int32_t 
com_ximeta_driver_NDASBusEnumerator::FindEmptySlotNumberOfNDASDevice(bool bRegistered)
{
	int32_t							slotNo = 0;
	OSMetaClassBase					*metaObject;
	com_ximeta_driver_NDASDevice	*NDASDevicePtr;
...
bool com_ximeta_driver_NDASBusEnumerator::createNDASDevice(PNDAS_ENUM_REGISTER	registerData, uint32_t status)
{
	bool								bret;
	...
	NDASDevicePtr->setDeviceIDString(registerData->DeviceIDString);
	NDASDevicePtr->setVID(registerData->VID);
	NDASDevicePtr->setStatus(status);
	NDASDevicePtr->setAutoRegister( (registerData->AutoRegister == 1 ? true : false) );
...
/*
	...
	addr.slpx_node[0] = 0;
	addr.slpx_node[1] = 0x0b;
	addr.slpx_node[2] = 0xd0;
	addr.slpx_node[3] = 0x02;
	addr.slpx_node[4] = 0x3b;
	addr.slpx_node[5] = 0x16;
	
	if (0 != memcmp(pnpCommand->fAddress, addr.slpx_node, 6)) {
		return;
	}
*/	
	...
	if (NDASDevicePtr) {
		...
		if(kNDASConfigurationDisable != NDASDevicePtr->configuration()
		   && !NDASDevicePtr->isInactive()) {
			...
			DbgIOLog(DEBUG_MASK_NDAS_WARNING, ("Not Enabled. %d\n", NDASDevicePtr->configuration()));
		}
	} else {
		DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("Not registered.\n"));
		...
		memcpy(&registerData.DevAddr.slpx_node, pnpCommand->fAddress, 6);
		registerData.DevAddr.slpx_port = htons(LPX_PORT_CONNECTION);
		
		registerData.Configuration = kNDASConfigurationEnable;
		registerData.SlotNo = FindEmptySlotNumberOfNDASDevice(false);
		
		if(false == createNDASDevice(&registerData, kNDASStatusDiscovered)) {
			DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("Can't Register!!!.\n"));		
		}
	}
 ...
 */
		...
		case kNDASDeviceNotificationNotPresent:
		{
			...
			DbgIOLog(DEBUG_MASK_PNP_TRACE, ("Receive kNDASDeviceNotificationNotPresent \n"));
			...
			switch (DiskArrayType) {
				...
				case NMT_INVALID:
				{
					DbgIOLog(DEBUG_MASK_PNP_INFO, ("[NotPresent] Invalid type\n"));
				}
					break;
				default:
				{
			...
			switch (previousType) {
				...
				case NMT_CDROM:
				{
					...
					if (NULL == logicalDevice) {
						...
						DbgIOLog(DEBUG_MASK_PNP_ERROR, ("=====================Before Delete Logical Device index %d\n", logicalDevice->Index()));
