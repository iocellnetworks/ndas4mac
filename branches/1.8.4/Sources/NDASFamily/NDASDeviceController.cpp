...
//				DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("workerThread: Command %d, this 0x%x\n", command->command(), this));
				...
				switch(command->command()) {
					...
					case kNDASCommandManagingIO:
					{
						...
						break;
					case kNDASCommandService:
					{
						com_ximeta_driver_NDASServiceCommand *serviceCommand = OSDynamicCast(com_ximeta_driver_NDASServiceCommand, command);
						
						processServiceRequest(serviceCommand);
					}
						break;
					default:
				...
				if ((currentTime - fProvider->lastTimeReceivingPnPMessage()) > NDAS_MAX_WAIT_TIME_FOR_PNP_MESSAGE) {
					DbgIOLog(DEBUG_MASK_NDAS_WARNING, ("No PnP Message for %d seconds.\n", (currentTime - fProvider->lastTimeReceivingPnPMessage())));
...
{
	DbgIOLog(DEBUG_MASK_NDAS_INFO, ("Entered. slotNumber[%d], status[%d]\n", fProvider->slotNumber(), fProvider->status()));
...
//	int count;
	...
	switch(fProvider->status()) {
		...
		case kNDASStatusDiscovered:
		{
			...
			DbgIOLog(DEBUG_MASK_NDAS_ERROR, ("Checkup... vid = %x\n", vid));
			...
			if (vid & NDAS_VID_AUTO_MASK) {
				...
				switch(vid) {
					case NDAS_VID_GENERAL_AUTO:
						fProvider->setUserPassword((char *)NDAS_OEM_CODE_GENERAL_AUTO.Bytes);
						result = kNDASStatusAutoRegister;
						break;
					case NDAS_VID_SOFT_NETDISK2_AUTO:
						fProvider->setUserPassword((char *)NDAS_OEM_CODE_SOFT_NETDISK2_AUTO.Bytes);
						result = kNDASStatusAutoRegister;
						break;
					case NDAS_VID_EMULATOR_AUTO:
						fProvider->setUserPassword((char *)NDAS_OEM_CODE_EMULATOR_AUTO.Bytes);
						result = kNDASStatusAutoRegister;
						break;
					default:
						// Not Supported!!!
...
//			result = kNDASStatusDiscovered;	// Recheck VID. 
...
void com_ximeta_driver_NDASDeviceController::processServiceRequest(com_ximeta_driver_NDASServiceCommand *command)
{
	PServiceCommand pServiceCommand;

	DbgIOLog(DEBUG_MASK_DISK_TRACE, ("Entered.\n"));

	pServiceCommand = command->serviceCommand();
	
	switch (pServiceCommand->serviceType) {
		case kNDASServiceCommandChangeUserPassword:
		{
			...
			switch(fProvider->status()) {
				...
				case kNDASStatusNeedRegisterKey:
					...
					break;
				default:
					DbgIOLog(DEBUG_MASK_DISK_ERROR, ("Bad Status %d.\n", fProvider->status()));					
					return;
			}
			
			if(kNDASStatusOnline != fProvider->changeUserPassword(pServiceCommand->password, pServiceCommand->vid, bChangeDevice)) {
				DbgIOLog(DEBUG_MASK_DISK_ERROR, ("changeUserPassword failed.\n"));
				
				goto Out;
			}
			
			// Change Write Key.
			fProvider->setWriteRight(pServiceCommand->bWritable);
		}
			break;
		default:
			break;
...
int com_ximeta_driver_NDASDeviceController::createUnitDevice(uint32_t targetNo, TARGET_DATA *Target)
{
	...
	if( NULL != (NDASPhysicalUnitDevicePtr = OSDynamicCast(com_ximeta_driver_NDASPhysicalUnitDevice, fUnits->findUnitDeviceByIndex(targetNo)) )) {
		...
		if (!NDASPhysicalUnitDevicePtr->createDataBuffer(fProvider->MaxRequestBytes())) {
			NDASPhysicalUnitDevicePtr->release();
...
//		NDASPhysicalUnitDevicePtr->setDevice(fProvider);
	...
	if (Target[targetNo].SectorSize < DISK_BLOCK_SIZE) {

		DbgIOLog(DEBUG_MASK_PNP_ERROR, ("Sector Size is too small %d\n", Target[targetNo].SectorSize));

		NDASPhysicalUnitDevicePtr->setStatus(kNDASUnitStatusFailure);
	} else {
...
int com_ximeta_driver_NDASDeviceController::updateUnitDevice(uint32_t targetNo, TARGET_DATA *Target, bool interfaceChanged, bool bPresent)
{
	...
	if (kNDASUnitStatusNotPresent == NDASPhysicalUnitDevicePtr->Status()
		|| kNDASUnitStatusFailure == NDASPhysicalUnitDevicePtr->Status()) {
