					  ...
					  )
{
	...
	if (bSupervisor) {
		memcpy(lsp_login_info.supervisor_password, password, 8);
	} else {
		memcpy(lsp_login_info.password, password, 8);
	}
	...
	{
		DbgIOLog(DEBUG_MASK_LS_INFO, ("lsp_login Failed(%d).\n", error));
		goto Out;
	}
	
	if(!discover && !bSupervisor)
	{
...
int lsp_wrapper_set_user_password(
								  lsp_wrapper_context_ptr_t		context,
								  uint8_t						*password
								  )
{
	...
	context->lsp_status = lsp_set_user_password(
												context->lsp_handle,
												password
												);
	ret = transfer_context(context);
	...
	context->lsp_status = lsp_reset(
									context->lsp_handle
									);
	ret = transfer_context(context);
	if(ret)
	{
		DbgIOLog(DEBUG_MASK_LS_ERROR, ("transfer_context Failed(%d) while reset.\n", ret));
		goto Out;
	}	
