...
uint32_t com_ximeta_driver_NDASDevice::lastTimeReceivingPnPMessage()
{
...
void com_ximeta_driver_NDASDevice::setStatus(uint32_t status)
{
	if(fStatus != status || kNDASStatusAutoRegister == status ) {	// For missing event of Auto Register.
		...
		if (fStatus != status) {
			DbgIOLog(DEBUG_MASK_NDAS_INFO, ("Change status of %d, [%d]->[%d]\n", slotNumber(), fStatus, status));
		}
...
void	com_ximeta_driver_NDASDevice::receiveServiceRequest(uint32_t serviceType, char *password, uint8_t vid, bool bWritable)
{

	// Create NDAS Command.
	com_ximeta_driver_NDASServiceCommand *command = OSTypeAlloc(com_ximeta_driver_NDASServiceCommand);
	...
	command->setCommand(serviceType, password, vid, bWritable);
...
int com_ximeta_driver_NDASDevice::getTargetData(TARGET_DATA *Targets, com_ximeta_driver_NDASPhysicalUnitDevice *unitDevice)
{
	...
	if (unitDevice) {
		...
		error = lsp_wrapper_login(lwcontext, false, userPassword(), 1, 0, 0);
		if(error) {
/*
			// Maybe the Discover port is busy. Try normal port.
			DbgIOLog(DEBUG_MASK_LS_ERROR, ("lsp_wrapper_login Retry...\n"));
			error = lsp_wrapper_login(lwcontext, false, userPassword(), 0, 0, 0);
			if(error) {
 */
...
Out:
	if( !unitDevice && lwcontext )
	{
		...
		lwcontext = NULL;				
	} else if (unitDevice && status != kNDASStatusOnline) {
		cleanupDataConnection(unitDevice, true);	
	}
...
int com_ximeta_driver_NDASDevice::discovery(TARGET_DATA *Targets)
{
	...
	if(result == kNDASStatusOnline) {
		...
		// Check VID.
		uint8_t vid = checkVID();
		
		if (vid != 0 
			&& vid != VID()
			&& autoRegister()) {
			// Need Register Key.
			result = kNDASStatusNeedRegisterKey;
		}
	}
...
int com_ximeta_driver_NDASDevice::getUnitDiskInfo(TARGET_DATA *Targets)
{
	...
	for(int count = 0; count < MAX_NR_OF_TARGETS_PER_DEVICE; count++)
	{
		...
		error = lsp_wrapper_login(lwcontext, false, userPassword(), 0, count, 0);
		if(error) {
...
int com_ximeta_driver_NDASDevice::loginCheck(char *password)
{
	DbgIOLog(DEBUG_MASK_LS_TRACE, ("Entered.\n"));
	...
	error = lsp_wrapper_login(lwcontext, false, password, 1, 0, 0);
	if(error) {
		/*
		 // Maybe the Discover port is busy. Try normal port.
		 DbgIOLog(DEBUG_MASK_LS_ERROR, ("lsp_wrapper_login Retry...\n"));
		 ...
		 if(error) {
		 */
		DbgIOLog(DEBUG_MASK_LS_INFO, ("lsp_wrapper_login Failed(%d)\n", error));
		status = kNDASStatusFailure;
		goto Out;
		//			}
	}
...
UInt8 com_ximeta_driver_NDASDevice::checkVID()
{
	...
	result = loginCheck((char *)NDAS_OEM_CODE_GENERAL_AUTO.Bytes);
	if (kNDASStatusOnline == result) {
		return NDAS_VID_GENERAL_AUTO;
	}

	// NDAS_OEM_CODE_GENERA
	result = loginCheck((char *)NDAS_OEM_CODE_GENERAL.Bytes);
	if (kNDASStatusOnline == result) {
		return NDAS_VID_GENERAL;
	}

	// NDAS_OEM_CODE_SOFT_NETDISK2_AUTO
	result = loginCheck((char *)NDAS_OEM_CODE_SOFT_NETDISK2_AUTO.Bytes);
	if (kNDASStatusOnline == result) {
		return NDAS_VID_SOFT_NETDISK2_AUTO;
	}
	
	// NDAS_OEM_CODE_EMULATOR
	result = loginCheck((char *)NDAS_OEM_CODE_SOFT_NETDISK2.Bytes);
	if (kNDASStatusOnline == result) {
		return NDAS_VID_SOFT_NETDISK2;
	}
	
	// NDAS_OEM_CODE_EMULATOR_AUTO
	result = loginCheck((char *)NDAS_OEM_CODE_EMULATOR_AUTO.Bytes);
	if (kNDASStatusOnline == result) {
		return NDAS_VID_EMULATOR_AUTO;
	}
	
	// NDAS_OEM_CODE_EMULATOR
	result = loginCheck((char *)NDAS_OEM_CODE_EMULATOR.Bytes);
	if (kNDASStatusOnline == result) {
		return NDAS_VID_EMULATOR;
	}
	
	return NDAS_VID_UNKNOWN;
}

int com_ximeta_driver_NDASDevice::changeUserPassword(char *newPassword, uint8_t newVid, bool bChageDevice)
{
	int						error;
	int						status = kNDASStatusOnline;
	...
	if (bChageDevice) {
		...
		error = lsp_wrapper_login(lwcontext, true, superPassword(), 0, 0, 0);
		if(error) {
			DbgIOLog(DEBUG_MASK_LS_ERROR, ("[%s] lsp_wrapper_login Failed(%d)\n", __FUNCTION__, error));
			status = kNDASStatusFailure;
			goto Out;
		...
		error = lsp_wrapper_set_user_password(lwcontext, (uint8_t*)newPassword);
		if(error) {
			DbgIOLog(DEBUG_MASK_LS_ERROR, ("[%s] lsp_wrapper_set_user_password Failed(%d)\n", __FUNCTION__, error));
			status = kNDASStatusFailure;
			goto Out;
		}
		DbgIOLog(DEBUG_MASK_LS_TRACE, ("%d = lsp_wrapper_set_user_password()\n", error));
		
		error = lsp_wrapper_logout(lwcontext, true);							// DO NOT logout!!!!!!!!!!!!!
		DbgIOLog(DEBUG_MASK_LS_TRACE, ("%d = lsp_wrapper_logout()\n", error));
		error = lsp_wrapper_disconnect(lwcontext);
		DbgIOLog(DEBUG_MASK_LS_TRACE, ("%d = lsp_wrapper_disconnect()\n", error));
		IOFree(lwcontext, lsp_wrapper_context_size());
	...
	DbgIOLog(DEBUG_MASK_LS_TRACE, ("Before IOSleep()\n"));

	IOSleep(5 * 1000);		// 5 sec.
	
	DbgIOLog(DEBUG_MASK_LS_TRACE, ("After IOSleep()\n"));
...
bool com_ximeta_driver_NDASDevice::initDataConnection(com_ximeta_driver_NDASPhysicalUnitDevice *device, bool writable)
{
	...
	error = lsp_wrapper_login(lwcontext, false, userPassword(), 0, device->unitNumber(), writable);
	if(error) {
		DbgIOLog(DEBUG_MASK_LS_ERROR, ("[%s] lsp_wrapper_login Failed(%d)\n", __FUNCTION__, error));
		goto Out;
	}
...
/*  IOMemoryMap *map = pScsiCommand->MemoryDescriptor_ptr->map(kIOMapDefaultCache);
	if(NULL == map) {
													  ...
													  )
{
	...
	// Mapping Buffer.
	pScsiCommand->MemoryDescriptor_ptr->retain();
/*	IOMemoryMap *map = pScsiCommand->MemoryDescriptor_ptr->map(kIOMapReadOnly);
	if(NULL == map) {
...
//	DbgIOASSERT(map);
	...
	// Check Parameter.
	//
	if(NumberOfBlocks > physicalDevice->maxRequestBytes() / physicalDevice->blockSize()) {
...
bool com_ximeta_driver_NDASDevice::sendIDECommand(PSCSICommand pCommand, com_ximeta_driver_NDASPhysicalUnitDevice *physicalDevice)
{
	...
	pCommand->MemoryDescriptor_ptr->prepare();
/*	
	IOMemoryMap *map = pCommand->MemoryDescriptor_ptr->map(kIOMapDefaultCache);
	if(NULL == map) {
		
		DbgIOLog(DEBUG_MASK_DISK_ERROR, ("Can't map buffer!!!\n"));

		pCommand->MemoryDescriptor_ptr->complete();
	...
	if(kSCSIDataTransfer_FromInitiatorToTarget == pCommand->DataTransferDirection) {
		...
		DbgIOLog(DEBUG_MASK_DISK_INFO, ("processSrbCommandPacket: ReadBytes %zu\n", result));
	}
	...
	iResult = lsp_wrapper_ide_packet_cmd(
		lwcontext,
		(PCDBd)pCommand->cdb,
		(kSCSIDataTransfer_NoDataTransfer == pCommand->DataTransferDirection) ? NULL : (char *)physicalDevice->DataBuffer(),
	...
	switch(iResult) {
		case 0:
		{
			...
			if(kSCSIDataTransfer_FromTargetToInitiator == pCommand->DataTransferDirection) {
				...
				DbgIOLog(DEBUG_MASK_DISK_INFO, ("processSrbCommandPacket: WriteBytes %zu\n", result));
			}
...
bool	com_ximeta_driver_NDASDevice::processManagingIOCommand(PManagingIOCommand pManagingIOCommand)
{
	...
	if (needLogin) {
		...
		error = lsp_wrapper_login(lwcontext, false, userPassword(), 0, physicalDevice->unitNumber(), writable);
		if(error) {
			DbgIOLog(DEBUG_MASK_LS_ERROR, ("[%s] lsp_wrapper_login Failed(%d)\n", __FUNCTION__, error));
			goto Out;
		}
