...
const IOExternalMethod NDASFamilyUserClientClassName::sMethods[kNDASNumberOfBusEnumeratorMethods] = {
	...
	{   // kNDASBusEnumeratorRegisterKey
		(IOService*) NULL,	 // The IOService * will be determined at runtime below.
		(IOMethod) &NDASFamilyUserClientClassName::RegisterKey,	// Method pointer.
		kIOUCStructIStructO,				// Scalar Input, Struct Input.
		sizeof(NDAS_ENUM_REGISTER_KEY),		// One scalar input value.
		sizeof(uint32_t)				// The size of the input struct.
	},
...
const IOExternalMethodDispatch NDASFamilyUserClientClassName::sMethods[kNDASNumberOfBusEnumeratorMethods] = {
	...
	{   // kNDASBusEnumeratorRegisterKey
		(IOExternalMethodAction) &NDASFamilyUserClientClassName::sRegisterKey,	// Method pointer.
		0,																		// No scalar input values.
		sizeof(NDAS_ENUM_REGISTER_KEY),													// The size of the input struct.
		0,																		// No scalar output values.
		sizeof(uint32_t)													// The size of the output struct.
		},
	{   // kNDASBusEnumeratorBind
...
IOReturn NDASFamilyUserClientClassName::openUserClient(void)
{
	...
	DbgIOLog(DEBUG_MASK_USER_INFO, ("Exited. result %d\n", result));
...
IOReturn NDASFamilyUserClientClassName::sRegisterKey(NDASFamilyUserClientClassName* target, void* reference, IOExternalMethodArguments* arguments)
{
	return target->RegisterKey((PNDAS_ENUM_REGISTER_KEY) arguments->structureInput,
							   (uint32_t*) arguments->structureOutput,
							   (IOByteCount) arguments->structureInputSize,
							   (IOByteCount*) &arguments->structureOutputSize);
}
...
IOReturn NDASFamilyUserClientClassName::RegisterKey(PNDAS_ENUM_REGISTER_KEY	registerKeyData,
													uint32_t				*registerKeyResult,
													IOByteCount				inStructSize,
													IOByteCount				*outStructSize)
{
	IOReturn result;
	...
	if (fProvider == NULL || isInactive()) {
		...
		result = fProvider->RegisterKey(registerKeyData, registerKeyResult, inStructSize, outStructSize);
	}
