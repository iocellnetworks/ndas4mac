...
#define round(x0,x1,ki)                                        \
    l1  = (rotr(x1, 4) ^ *(((uint32_t*)key) + ki + 1));   \
    l0  = (x1 ^ *(((uint32_t*)key) + ki));                \
...
#define round(x0,x1,ki)                                                     \
    l1  = (rotr(x1, 4) ^ *(((uint32_t*)key) + ki + 1)) & 0x3f3f3f3f;   \
    l0  = (x1 ^ *(((uint32_t*)key) + ki)) & 0x3f3f3f3f;                \
    x0 ^= sx_tab[0][byte(l0,0)] | sx_tab[1][byte(l1,0)]                     \
        | sx_tab[2][byte(l0,1)] | sx_tab[3][byte(l1,1)]                     \
        | sx_tab[4][byte(l0,2)] | sx_tab[5][byte(l1,2)]                     \
        | sx_tab[6][byte(l0,3)] | sx_tab[7][byte(l1,3)]
...
void des_ec32(const void *i_blk, void *o_blk, void *key);
void des_dc32(const void *i_blk, void *o_blk, void *key);
