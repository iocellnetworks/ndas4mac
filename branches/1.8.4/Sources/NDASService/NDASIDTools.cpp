	...
	if((ret = DecryptSerial(&info)) == TRUE &&
	   ...
	   (strlen(wKey) == 0 || (strlen(wKey) == 5 && info.bIsReadWrite == TRUE))) {
		...
		if (NDAS_ID_EXTENSION_GENERAL.VID == info.ucVid
			...
			// Default ID.
		} else if (NDAS_ID_EXTENSION_SOFT_NETDISK2.VID == info.ucVid
				   && NDAS_ID_EXTENSION_SOFT_NETDISK2.Seed == info.ucRandom
				   && NDAS_ID_EXTENSION_SOFT_NETDISK2.Reserved[0] == info.reserved[0]
				   && NDAS_ID_EXTENSION_SOFT_NETDISK2.Reserved[1] == info.reserved[1]) {
			// Soft Netdisk ID.
		} else if (NDAS_ID_EXTENSION_EMULATOR.VID == info.ucVid
				   && NDAS_ID_EXTENSION_EMULATOR.Seed == info.ucRandom
				   && NDAS_ID_EXTENSION_EMULATOR.Reserved[0] == info.reserved[0]
				   && NDAS_ID_EXTENSION_EMULATOR.Reserved[1] == info.reserved[1]) {
			// Emualtor ID.
