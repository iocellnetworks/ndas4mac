...
//					if (parameter.cfsName)	CFRelease(parameter.cfsName);
			...
			case NDAS_NOTIFICATION_REQUEST_REFRESH:
			{
				...
				break;
			case NDAS_NOTIFICATION_REQUEST_REGISTER_KEY:
			{
				NDASPreferencesParameter	parameter;
				int							retry;
				bool						result;
				...
				if ( 1 != NDASPreferencesQueryBySlotNumber(&parameter) ) {
					...
					if ( KERN_SUCCESS != NDASIOConnectRegisterKey(&parameter) ) {
						errorCode = NDAS_NOTIFICATION_ERROR_FAIL;
					} else {
						errorCode = NDAS_NOTIFICATION_ERROR_SUCCESS;
						
						NDASPreferencesChangeAutoRegister(slotNumber, false);
						
						// Wait Reset time...
						sleep(5);	// 5 sec.
						
						NDASPhysicalDeviceInformation physicalParameter = { 0 };
						
						physicalParameter.slotNumber = parameter.slotNumber;
						retry = 5;
											
						while((result = NDASGetInformationBySlotNumber(&physicalParameter)) 
							  && retry > 0 
							  && physicalParameter.status != kNDASStatusOnline) {
							retry--;
							sleep(1);	// Wait 1 sec.							
						}
						...
						memcpy(unitDeviceNotify.DeviceId, physicalParameter.address, 6);
						unitDeviceNotify.UnitNo = 0;
...
static void MakeOneLocalization( CFStringRef key, CFStringRef *value  )
{
    ...
    CFStringRef		errStr;
    SInt32			errCode;
	CFStringRef		item = CFSTR("English");	
	...
	if(NULL != bundle) {
		...
		{ 
			if ( osVersion >= 0x1050 ) 
			{
				...
				for(int i = 0; i < CFArrayGetCount(localeArray); i++) {
					if (kCFCompareEqualTo == CFStringCompare(CFSTR("en"), (CFStringRef)CFArrayGetValueAtIndex(localeArray, i), 0)) {
						item = CFSTR("English");
						break;
					} else if (kCFCompareEqualTo == CFStringCompare(CFSTR("ko"), (CFStringRef)CFArrayGetValueAtIndex(localeArray, i), 0)) {
						item = CFSTR("ko");
						break;
					} else if (kCFCompareEqualTo == CFStringCompare(CFSTR("de"), (CFStringRef)CFArrayGetValueAtIndex(localeArray, i), 0)) {
						item = CFSTR("German");
						break;
					} else if (kCFCompareEqualTo == CFStringCompare(CFSTR("ru"), (CFStringRef)CFArrayGetValueAtIndex(localeArray, i), 0)) {
						item = CFSTR("ru");
						break;
					} 
				}
		...
		if (url && CFURLCreateDataAndPropertiesFromResource( kCFAllocatorDefault,
			...
			stringTable = (CFDictionaryRef)CFPropertyListCreateFromXMLData( kCFAllocatorDefault,
		...
		if( url)
			CFRelease(url);
		if( stringTable)
			*value = CFStringCreateCopy(kCFAllocatorDefault, (CFStringRef)CFDictionaryGetValue(stringTable, key));
	}
	
    if (!*value)
        *value = CFStringCreateCopy(kCFAllocatorDefault, key);
/*	
    {
									   ...
									   void * messageArgument )
{
	...
	switch (messageType) {
		case kNDASFamilyIOMessageNDASDeviceStatusWasChanged:
		{
			...
			syslog(LOG_INFO, "DeviceDriverNotificationCallback: slotNumber %d", slotNumber);
			...
			if (kNDASStatusAutoRegister == infos.status) {
				...
				switch(infos.vid) {
					case NDAS_VID_SOFT_NETDISK2_AUTO:
					{
						memcpy(serialInfo.ucAddr, infos.address, 6);
						serialInfo.ucRandom = NDAS_ID_EXTENSION_SOFT_NETDISK2.Seed;
						serialInfo.ucVid = (infos.vid & ~NDAS_VID_AUTO_MASK);		// Display Not autoregister key.
						serialInfo.reserved[0] = NDAS_ID_EXTENSION_SOFT_NETDISK2.Reserved[0];
						serialInfo.reserved[1] = NDAS_ID_EXTENSION_SOFT_NETDISK2.Reserved[1];						
						bSupport = TRUE;
					}
						break;
					case NDAS_VID_EMULATOR_AUTO:
					{
						serialInfo.ucRandom = NDAS_ID_EXTENSION_EMULATOR.Seed;
						serialInfo.ucVid = (infos.vid & ~NDAS_VID_AUTO_MASK);		// Display Not autoregister key.
						serialInfo.reserved[0] = NDAS_ID_EXTENSION_EMULATOR.Reserved[0];
						serialInfo.reserved[1] = NDAS_ID_EXTENSION_EMULATOR.Reserved[1];
						bSupport = TRUE;
					}
						break;
				...
				if (bSupport) {
					...
					MakeOneLocalization(CFSTR("NDAS Device (%s)"), &cfsName);
					parameter.cfsName = CFStringCreateWithFormat(NULL, NULL, cfsName, parameter.chID[0]);
			...
			switch(pHixHeader->Type) {
				case NHIX_TYPE_DISCOVER:
				{
					...
					for(int count = 0; count < pDiscover->Data.EntryCount; count++) {
						...
						indexArray = NDASGetInformationCreateIndexArray(NDAS_DEVICE_CLASS_NAME);
									...
									if(pRequest->Data.Entry[0].AccessType == NHIX_UDA_RW 
									   || pRequest->Data.Entry[0].AccessType == NHIX_UDA_WO ) {
										...
										if (name) {
											...
											MakeOneLocalization(CFSTR("The host '%@' is requesting the Write Access of '%@'."), &cfsHeaderFrame);
